...
def process_data_raw_dir():
    ...
    for dir_raw_name in os.listdir(DATA_RAW_DIR):
        ...
        if os.path.isdir(dir_raw_path):
            ...
            json_all_file_path = os.path.join(dir_processed_path, json_all_file_name)

            if os.path.exists(json_all_file_path):
                with open(json_all_file_path, "r") as f:
                    ALL_RESULTS = json.load(f)
            else:
                ALL_RESULTS = {}
                with open(json_all_file_path, "w") as file_all_json:
    ...
    with open(json_all_file_path, "w") as json_all:
