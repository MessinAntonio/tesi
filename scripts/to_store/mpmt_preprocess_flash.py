...
string += f" - Digita 's' per fermare."
print(string)

while flashEnable:
    idx_file = input("Digitare:")
    ...
    for keys, values in intervals.items():
        ...
        results_table.field_names = ["Segnale", "Canale", "Media", "Dev Std", "N eventi"]
        ...
        for idx, ch_id in enumerate(channels): 
            ...
            if df_ch.empty:
                ...
                mean_val = np.mean(df_ch["adc"])
                std_val  = np.std(df_ch["adc"])
                count_val = len(df_ch)

            results_table.add_row([
                keys,
                f"{ch_id:02d}",
                f"{mean_val:.2f}" if mean_val != "-" else "-",
                f"{std_val:.2f}" if std_val != "-" else "-",
                count_val
            ])
        ...
        print("\n\n=== RISULTATI STATISTICI ---", keys.upper(), "===\n")
        print(results_table)
        print("\n")